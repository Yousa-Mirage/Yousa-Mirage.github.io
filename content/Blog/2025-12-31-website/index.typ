...
#image("devices.webp", width: 620pt)
...
配合优雅的*衬线字体*与*类纸张背景*，Tufte 风格成功在数字屏幕上复刻了如阅读纸质著作般清晰、优雅、沉浸的阅读体验。

=== 什么是 Typst？

传统的网页开发要么需要你同时学会 HTML（结构）、CSS（样式）和 JavaScript（交互）。但在这个模板里，你只需要写 Typst，一种比 LaTeX 更简单、比 Markdown 更强大的排版语言，我们利用它实验性的 HTML 导出功能来编译网页。

Typst 是一个新兴的、现代化的标记语言排版系统，由 Rust 编写，可以输出 PDF、HTML 等丰富的文档格式。它旨在成为 LaTeX 的现代化替代品，简单易学、编译极快、使用友好，能够轻松实现极为精美和丰富的排版效果。

Typst 的语法非常接近自然语言。你想加个标题？写 `= 标题`。想加个编号列表？写 `+ 列表项1`。想添加个图片？直接写 `#image(你的图片文件)`。使用标记语言编写内容，可以让你专注于内容本身，而不是纠结排版细节。

（煮啵的日常写作已经 All in Typst 了，用 Typst 写论文、写笔记、写作业、写博客、做 PPT...）

#figure(caption: "使用 Zed 编辑器编写示例文档")[#image("zed.png", width: 600pt)]
...
这个模板只依赖 Typst 和 Python，Typst 是我们的核心排版引擎，Python 是为了自动构建和预览。你只需要安装这两个工具即可。
...
- _煮啵煮啵，我从来没用过 Git 和 GitHub，你能教教我吗？_
  - 能的兄弟，能的。我在文档里写好了 什么是 Git、Git 怎么用，你只需要学会使用“提交”操作来存档修改、使用“推送”操作来上传修改两个操作就行了！我还链接了我很喜欢的#link("https://www.bilibili.com/video/BV1Hkr7YYEh8")[“给傻子的Git教程”]，还不会就看它！
...
`content/` 文件夹中里面有很多 `.typ` 文件，这些就是我写的 Typst 示例文档。随便打开一个，试着改几个字，保存，重新运行编译命令。你会发现，网页自动变成了你修改后的样子！我在模板里留了很多现成的例子，你想改主页、写博客、放简历，直接照着样例写就行。

例如，得益于 Typst 强大的排版能力和丰富的输出格式，你只需要简单编写就能创建一份排版精美的简历，然后既能生成方便阅读的网页版，又可同时提供排版固定、下载方便的 PDF 版。如果你著作等身，也可以为你的每篇发表都添加 PDF 文件链接，将 PDF 文件一并上传到网站，让读者轻松下载你的高作。

- _煮啵煮啵，你说了半天 Typst，我完全不会写啊，有没有学习资料呢？_
  - 有的兄弟，有的。我编写了“Typst 快速入门资料”和“Typst 功能速览与样例”两份文档，在里面详细介绍了 *什么是 Typst、为什么本项目使用 Typst、怎么快速学习 Typst* 等内容。你需要学习的 Typst 知识非常少，学完这些知识就能使用 Typst 编写你自己的文档和网页了！此外，我已经把网站常用的功能都写在了“Typst 功能速览与样例”里，照着写就可以！

=== 让全世界看到你的网站

- _煮啵煮啵，我在电脑上准备好了我的网站，有没有办法让别人也能访问啊？_
  - 有的兄弟，有的。得益于 GitHub Actions 和 GitHub Pages，你不需要购买服务器或域名，就可以免费将你的网站自动发布在 `<your-github-name>.github.io` 这个网站上。我已经准备好了 GitHub 工作流，还写好了关于 *如何使用 GitHub Pages 免费部署你的网站* 的文档。你只需要读一读，然后把你的文件上传（推送）到 GitHub，你的网站就会自动生成、部署、更新了。

你需要独立进行的工作可能就只 有注册 GitHub 账号 和 下载你自己喜欢的代码编辑器 两项。注册账号，下载 VS Code，安装 Tinymist 扩展，打开你的网站项目。没有了，应该真的没有了！

（即使你不使用本项目构建网站，这些文档和教程也能让你学到很多！）

更加详细的内容和构建说明，请阅读项目仓库中的 README 和 Wiki 文档。


== 总结

开发 Tufted Blog Template 项目的初衷，就是因为我发现很多同学被技术门槛或畏惧心理挡在了门外，错失了拥有个人网站的乐趣。碰巧地，我发现了 Tufted 包和 Tufte 风格，在简洁、美观、富有高级感的同时提供了极佳的阅读体验，非常适合构建学术向的个人网站。于是便有了我的网站，于是便有了这个模板项目。
...
如果你遇到任何问题，欢迎留言或在 GitHub 中提问、讨论和反馈。
